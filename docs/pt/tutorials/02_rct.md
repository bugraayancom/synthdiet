# Projetar e executar um ECR

Este tutorial usa o motor `synthdiet.trials` para simular um ensaio
clínico randomizado de braços paralelos.
...
print(f"São necessários {n} pacientes por braço (diferença 2 kg, poder 80%).")
```
...
- **ITT**: analisa cada paciente no braço atribuído (incluindo
  abandonos); mede **efetividade**.
- **Por protocolo**: somente pacientes acima do limiar de adesão;
  mede **eficácia**.
...
ax = consort_diagram(result, title="Ensaio DASH sintético")
ax.figure.savefig("consort.png", dpi=150, bbox_inches="tight")
```

## ANCOVA ajustada à linha de base
...
`CrossoverTrial` (cruzado) e `FactorialTrial` (fatorial 2×2)
compartilham a mesma interface. Detalhes na referência da API.
