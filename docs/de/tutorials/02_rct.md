# RCT entwerfen und durchführen

Dieses Tutorial nutzt die `synthdiet.trials`-Engine zur Simulation
einer randomisierten kontrollierten Studie (RCT) mit Parallelarmen.
...
print(f"{n} Patient*innen pro Arm (2-kg-Differenz, 80% Power).")
```

## Studie definieren und starten
...
- **ITT**: alle Patient\*innen im zugewiesenen Arm, einschließlich
  Drop-outs; misst die **Effectiveness**.
- **PP**: nur Patient\*innen oberhalb der Adhärenzschwelle; misst die
  **Efficacy**.
...
ax = consort_diagram(result, title="Synthetische DASH-Studie")
...
`CrossoverTrial` (Crossover) und `FactorialTrial` (2×2-faktoriell)
nutzen dieselbe Schnittstelle wie `ParallelTrial`. Details in der
API-Referenz.
